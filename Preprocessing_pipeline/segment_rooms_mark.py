        ...
        if point_room_ids[i]:
            ...
            if image:
                ...
                if room_id not in workable_rooms:
                    ...
                    first_label += 1
                new_room_id = correct_order_map[room_id]
                src_path = source_dir / f"{image[0]}"
                out_path = out_dir / f"room_{new_room_id:03d}"
...
def run_split(
    ...
    for i in range(len(correct_order_map)):
        unique_labels = np.append(unique_labels, i+1)
...
def parse_args():
    ...
    p.add_argument("--obj_types", type=str, nargs="+", default=["chair","couch","curtain","door","monitor","plant","table","window", "combined_non_structural"])
...
if __name__ == "__main__":
    ...
    run_split(
        ...
        slice_min=ground_floor_level[0] + 0.18, # Change this between house and uni
        slice_max=ceiling_level[0] - 0.45, # Change this between house and uni
        grid_res=args.grid,
